/**
 * DRAFT Order Shipped Email â€“ visual test only.
...
<head>
  ...
  <title>Order Shipped â€“ SkyGloss</title>
  <!--[if mso]>
  <style type="text/css">
        ...
        <table role="presentation" width="${WIDTH}" cellpadding="0" cellspacing="0" border="0" style="width:${WIDTH}px; max-width:${WIDTH}px;">
          <tbody><tr>
            <td style="padding:16px 8px; font-family: Arial, Helvetica, sans-serif; font-size:11px; color:#888888; text-align:center;">
              DRAFT TEMPLATE â€“ for testing only. Not the live order shipped email.
            </td>
          </tr>
        </tbody></table>
...
async function sendDraftOrderShippedEmail() {
  ...
  const mailOptions = {
    ...
    to: 'it@skygloss.com',
    subject: `[DRAFT TEST] Order Shipped â€“ ${sampleOrder.orderNumber} â€“ ${stamp}`,
    html,
  };
