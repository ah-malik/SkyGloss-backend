...
const ASSETS = {
  ...
  // Combined Signature + Car (email-safe single image)
  signatureCar:
...
function buildDraftWelcomeHtml(details) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  ...
  <title>Welcome to SkyGloss</title>
  <!--[if mso]>
  <style type="text/css">
...
async function sendDraftWelcomeEmail() {
  ...
  const html = buildDraftWelcomeHtml(userDetails);

  const previewPath = path.join(__dirname, 'welcome-email-draft-preview.html');
  ...
  console.log('Draft welcome email sent to it@skygloss.com');
...
sendDraftWelcomeEmail().catch((error) => {
  console.error('Failed to send draft welcome email:', error.message);
  process.exit(1);
});
