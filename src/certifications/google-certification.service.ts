...
export class GoogleCertificationService {
    ...
    async portToGoogleSheet(certification: Certification, status: string = 'UNPAID') {
        ...
        const certData = (certification as any).toObject ? (certification as any).toObject() : certification;
        ...
        try {
            ...
            logEntry(`[GoogleSheet] SUCCESS! Status: ${response.status}, Data: ${JSON.stringify(response.data)}`);
        } catch (error: any) {
